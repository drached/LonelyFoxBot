...
class Music(commands.Cog, wavelink.WavelinkMixin):
    ...
    async def cog_check(self, ctx):
        if isinstance(ctx.channel, discord.DMChannel):
            await ctx.send("dat funzt so net xD")
            return False
    ...
    async def connect_command(self, ctx, *, channel: t.Optional[discord.VoiceChannel]):   # semingly cant connect to a given channel dont know why
    ...
    async def stop_command(self, ctx):
        ...
        player.queue.empty()
        await player.stop()
        await ctx.send("Playback is stopped.")
    ...
    async def queue_command_error(self, ctx, exc):
        if isinstance(exc, QueueIsEmpty):
            await ctx.send("The queue is empty at the moment.")
